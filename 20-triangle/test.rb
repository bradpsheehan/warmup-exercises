...
class TriangelTest < MiniTest::Unit::TestCase
  ...
  def test_triangles_with_no_size_are_illegal
    assert_raises(TriangleError) do triangle(0, 0, 0) end
  end

  def test_triangles_with_negative_sides_are_illegal
    assert_raises(TriangleError) do triangle(3, 4, -5) end
  end

  def test_triangles_are_illegal_if_they_cannot_have_straight_sides
    assert_raises(TriangleError) do triangle(1, 1, 3) end
  end

  def test_triangles_cannot_have_too_short_legs
    assert_raises(TriangleError) do triangle(2, 4, 2) end
  end
end
